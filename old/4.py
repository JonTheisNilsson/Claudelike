...
def draw_grid(surface, offset_x, offset_y, width, height):
    ...
    start_x = max(0, offset_x // GRID_CELL_SIZE)
    end_x = min(MAP_SIZE[0], (offset_x + width) // GRID_CELL_SIZE + 1)
    start_y = max(0, offset_y // GRID_CELL_SIZE)
    end_y = min(MAP_SIZE[1], (offset_y + height) // GRID_CELL_SIZE + 1)

    # Draw vertical lines
    for x in range(start_x, end_x):
        pygame.draw.line(surface, WHITE, 
                         (x * GRID_CELL_SIZE - offset_x, max(0, -offset_y)), 
                         (x * GRID_CELL_SIZE - offset_x, min(height, MAP_SIZE[1] * GRID_CELL_SIZE - offset_y)))
    
    # Draw horizontal lines
    for y in range(start_y, end_y):
        pygame.draw.line(surface, WHITE, 
                         (max(0, -offset_x), y * GRID_CELL_SIZE - offset_y), 
                         (min(width, MAP_SIZE[0] * GRID_CELL_SIZE - offset_x), y * GRID_CELL_SIZE - offset_y))
...
def update_camera(window_size):
    ...
    if window_size[0] >= map_pixel_width:
        ...
        target_offset_x = player_pos[0] * GRID_CELL_SIZE - window_size[0] // 2
        camera_offset[0] = max(0, min(target_offset_x, map_pixel_width - window_size[0]))

    if window_size[1] >= map_pixel_height:
        camera_offset[1] = (window_size[1] - map_pixel_height) // 2
    else:
        target_offset_y = player_pos[1] * GRID_CELL_SIZE - window_size[1] // 2
        camera_offset[1] = max(0, min(target_offset_y, map_pixel_height - window_size[1]))
